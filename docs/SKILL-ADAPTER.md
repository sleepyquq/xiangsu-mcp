...
适用：像塑 9.4.0、MCP 0.2.0；核对日期 2026-09-08。这是适配契约，不是全部能力已通过真实编辑器回归的声明。修改适配器后同步更新本文件；以已加载服务的工具 schema 和编辑器实际读回为准。历史 9.3.2 验证不能直接继承为 9.4 验证。
...
- 音频模拟驱动 AgentAudioDetector / AgentAudioKeyword 的检测结果与回调，不生成真实声音。volume 是原始幅度，脚本 getVolume() 仍按官方曲线转换为感知响度。本桥接输入从渲染线程接收时计时，并等待 audioInput 确认；确认超时会报错而不是报告已执行。每步结束清空输入；没有对应运行库时在动作执行前拒绝。
